...
from src.pricing.price_reference import get_all_subsidy_data, find_price_by_trim, find_price_by_key
from src.config.constants import (
    PricingConfig, YEAR_INFO, SUBSCRIPTION_TERMS, CostStructure
)
...
def add_subsidy_columns_to_df(df):
    ...
    if not subsidy_df.empty:
        ...
        df['subsidy_national'] = -(merged_df['national'].fillna(0) * 10000)  # 음수로 변환
        df['subsidy_lease'] = -(merged_df['lease'].fillna(0) * 10000)   # 음수로 변환
        
        # 전기차 세금 보조금
        df['subsidy_tax'] = df['fuel'].apply(lambda x: -PricingConfig.ELECTRIC_TAX_SUBSIDY if x == "전기" else 0)
    else:
        # 기본값 설정
...
def calculate_car_cost(car_price, fuel_type="", subsidy_trim=""):
    """차량 비용 계산"""
    # 보조금 계산
...
def get_cost_structure():
    """비용 구조 반환"""
    return sum(CostStructure.INITIAL_COSTS.values()), sum(CostStructure.RECURRING_YEARLY_COSTS.values())
...
def calculate_subscription_return_fee(year_label, discounted_costs, residual_values):
    ...
    subscription_fee = (numerator / denominator) * PricingConfig.RETURN_TYPE_LOSS / 10000
    return math.ceil(subscription_fee) * 10000


def calculate_early_repayment_fees_by_term(car_cost, down_payment, annual_rate, total_months):
    """중도상환 수수료 계산"""
    early_fees = {}
...
def cost_own_by_year(n_years, down_payment, init_setup_cost, installment_payment_yearly, 
                    recurring_cost_1y, early_repayment_fees_by_term):
    """인수형 연도별 비용 계산"""
...
def calculate_subscription_own_fee(year_label, down_payment, init_setup_cost, 
                                 installment_payment_yearly, recurring_cost_1y, 
    ...
    subscription_fee = (numerator / denominator) * PricingConfig.PURCHASE_TYPE_LOSS / 10000
    return math.ceil(subscription_fee) * 10000
...
def calculate_option_fees(price_options):
    ...
    for term in required_terms:
        ...
        discount_sum = sum(YEAR_INFO[f"Y{j}"]["discount"] for j in range(1, year_count + 1))
...
def calculate_subscription_fees(car_price, fuel_type="", subsidy_trim=""):
    """구독료 계산 메인 함수"""
    # 차량 비용 계산
    car, car_cost = calculate_car_cost(car_price, fuel_type, subsidy_trim)
...
def calculate_pricing(df):
    ...
    df["price_tax"] = df.apply(lambda row: 
        (float(row["price_car_tax_post"]) if pd.notna(row["price_car_tax_post"]) and row["price_car_tax_post"] != "?" else float(row["price_car_original"])) * 0.07, axis=1)
    df["price_total"] = df.apply(lambda row: 
        (float(row["price_car_tax_post"]) if pd.notna(row["price_car_tax_post"]) and row["price_car_tax_post"] != "?" else float(row["price_car_original"])) + 
        row["price_tax"] + row["subsidy_national"] + row["subsidy_lease"] + row["subsidy_tax"] + PricingConfig.REGISTRATION_FEE, axis=1)
    ...
    for idx, row in df.iterrows():
        try:
            ...
            price_car_original = float(row["price_car_original"]) if pd.notna(row["price_car_original"]) and row["price_car_original"] != "?" else 0
    ...
    for idx, row in df.iterrows():
        ...
        car_price = float(row["price_car_tax_post"]) if pd.notna(row["price_car_tax_post"]) and row["price_car_tax_post"] != "?" else float(row["price_car_original"])
        ...
        fees = calculate_subscription_fees(car_price, fuel_type, subsidy_trim)
    ...
    df["fee_care"] = df["fuel"].apply(lambda x: PricingConfig.CARE_FEE_ELECTRIC if x == "전기" else PricingConfig.CARE_FEE_OTHER)
